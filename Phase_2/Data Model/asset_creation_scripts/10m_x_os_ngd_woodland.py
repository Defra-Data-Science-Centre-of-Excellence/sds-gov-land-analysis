...
#Trees
unique_values_df = ngd_land.select("oslandcovertiera").distinct()

# Show the unique values
unique_values_df.display()

# COMMAND ----------

filtered_df = ngd_land.filter(ngd_land["oslandcovertiera"] == "Trees")
unique_values_df = filtered_df.select("oslandcovertierb").distinct()
...
focal_name = "habnet_aonb_2" # used to name the layer in outputs
focal_path = "dbfs:/mnt/base/unrestricted/source_defra_data_services_platform/dataset_areas_of_outstanding_natural_beauty/format_GEOPARQUET_areas_of_outstanding_natural_beauty/LATEST_areas_of_outstanding_natural_beauty/Areas_of_Outstanding_Natural_Beauty_England.parquet"
#focal_column = 'prmry_h'
#focal_variable = "Broadleaved, Mixed and Yew Woodland"

# COMMAND ----------

focal_layer = sedona.read.format("geoparquet").load(focal_path)

#focal_layer = focal_layer.filter(focal_layer[focal_column] == focal_variable) # Optional Filter statement
